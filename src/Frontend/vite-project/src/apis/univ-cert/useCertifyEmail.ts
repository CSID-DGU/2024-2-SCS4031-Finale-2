...
async function certifyEmail({
  email,
  univName,
}: CertifyEmailProps): Promise<UnivCertResponse> {
...
const useCertifyEmail = (): UseMutationResult<
  UnivCertResponse,
  Error,
  CertifyEmailProps
> => {
  return useMutation<UnivCertResponse, Error, CertifyEmailProps>({
    mutationFn: ({ email, univName }: CertifyEmailProps) =>
      certifyEmail({ email, univName }),
  });
};
