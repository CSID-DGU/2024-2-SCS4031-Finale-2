...
로 리다이렉트
	...
	public RedirectView loginKakao(@AuthenticationPrincipal OAuth2User oAuth2User) {
		...
		String targetUrl;
		if (isSignup) {
			targetUrl = "http://localhost:5173?accessToken=" + jwtToken.getAccessToken() + "&refreshToken=" + jwtToken.getRefreshToken();
		} else {
			targetUrl = "http://localhost:5173/signup?accessToken=" + jwtToken.getAccessToken() + "&refreshToken=" + jwtToken.getRefreshToken();
		}
